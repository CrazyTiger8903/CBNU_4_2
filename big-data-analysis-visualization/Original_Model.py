...
성능이 줄어버림...
Train Remainset Accuracy (others): 78.29%
Train Forgetset Accuracy (deer): 76.74%
Test Remainset Accuracy (others): 73.29%
Test Forgetset Accuracy (deer): 70.00%
...
transform = transforms.Compose([
    ...
    transforms.RandomVerticalFlip(p=0.2),    # 20% 확률로 이미지를 상하 반전
    transforms.RandomCrop(32, padding=4),   # 이미지 크기를 32x32로 유지하면서 패딩 4를 추가한 뒤 무작위 자르기
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),  # 밝기, 대비, 채도, 색조 조정
    transforms.RandomRotation(15),          # ±15도 범위에서 이미지를 무작위로 회전
    transforms.RandomGrayscale(p=0.1),      # 10% 확률로 이미지를 그레이스케일로 변환
    transforms.GaussianBlur(kernel_size=(5, 5), sigma=(0.1, 2.0)),  # 랜덤 가우시안 블러
